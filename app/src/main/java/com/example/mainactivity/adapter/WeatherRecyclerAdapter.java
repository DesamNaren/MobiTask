...
public class WeatherRecyclerAdapter extends RecyclerView.Adapter<WeatherViewHolder> {
    ...
    public WeatherRecyclerAdapter(Context context, List<WeatherInfo> itemList) {
    ...
    public void onBindViewHolder(WeatherViewHolder customViewHolder, int i) {
        WeatherInfo weatherInfoItem = itemList.get(i);

        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);

        // Temperature
        float temperature = UnitConvertor.convertTemperature(Float.parseFloat(weatherInfoItem.getTemperature()), sp);
        if (sp.getBoolean("temperatureInteger", false)) {
            temperature = Math.round(temperature);
        }

        // Rain
//        double rain = Double.parseDouble(weatherInfoItem.getRain());
//        String rainString = UnitConvertor.getRainString(rain, sp);

        // Wind
        double wind;
        try {
            wind = Double.parseDouble(weatherInfoItem.getWind());
        } catch (Exception e) {
            e.printStackTrace();
            wind = 0;
        }
        ...
        // Pressure
        double pressure = UnitConvertor.convertPressure((float) Double.parseDouble(weatherInfoItem.getPressure()), sp);
        ...
        try {
            ...
            dateString = resultFormat.format(weatherInfoItem.getDate());
        ...
        if (sp.getBoolean("differentiateDaysByTint", false)) {
            ...
            if (weatherInfoItem.getNumDaysFrom(now) > 1) {
                ...
                if (weatherInfoItem.getNumDaysFrom(now) % 2 == 1) {
        ...
        customViewHolder.itemDescription.setText(weatherInfoItem.getDescription().substring(0, 1).toUpperCase() +
                weatherInfoItem.getDescription().substring(1) );
        Typeface weatherFont = Typeface.createFromAsset(context.getAssets(), "fonts/weather.ttf");
        customViewHolder.itemIcon.setTypeface(weatherFont);
        customViewHolder.itemIcon.setText(weatherInfoItem.getIcon());
        if (sp.getString("speedUnit", "m/s").equals("bft")) {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " +
                    UnitConvertor.getBeaufortName((int) wind) + " " + MainWeatherActivity.getWindDirectionString(sp, context, weatherInfoItem));
        } else {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " + new DecimalFormat("#.0").format(wind) + " " +
                    MainWeatherActivity.localize(sp, context, "speedUnit", "m/s")
                    + " " + MainWeatherActivity.getWindDirectionString(sp, context, weatherInfoItem));
        }
        customViewHolder.itemPressure.setText(context.getString(R.string.pressure) + ": " + new DecimalFormat("#.0").format(pressure) + " " +
                MainWeatherActivity.localize(sp, context, "pressureUnit", "hPa"));
        customViewHolder.itemHumidity.setText(context.getString(R.string.humidity) + ": " + weatherInfoItem.getHumidity() + " %");
    }
